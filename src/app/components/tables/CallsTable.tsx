...
const CallsTable = ({ data = [] }: CallsTableProps) => {
  ...
  const uniqueLocations = useMemo(() => 
    Array.from(new Set(data.map(call => call.location_name).filter(Boolean))),
    [data]
  )

  const uniqueStatuses = useMemo(() => 
    Array.from(new Set(data.map(call => call.phone_call_status).filter(Boolean))),
    [data]
  )

  const uniqueTypes = useMemo(() => 
    Array.from(new Set(data.map(call => call.phone_direction).filter(Boolean))),
    [data]
  )
  ...
  const columns: ColumnDef<Call>[] = [
    ...
    {
      accessorKey: 'phone_direction',
      header: 'Tipo',
    },
    {
      accessorKey: 'phone_call_status',
      header: 'Estado',
    },
    {
